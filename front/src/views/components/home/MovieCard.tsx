...
export default function MovieCard({ video }: { video: Movie }) {
  ...
  const toggleFavorite = async (e: React.MouseEvent) => {
    ...
    try {
      if (isFavorite) {
        await api({
          ...apiRoutes.movies.favorite.remove,
          path: apiRoutes.movies.favorite.remove.path
            .replace('{movie_id}', video.id.toString())
        });
        setIsFavorite(false);
        toast.success('Film retiré des favoris');
      } else {
        await api({
          ...apiRoutes.movies.favorite.add,
          path: apiRoutes.movies.favorite.add.path
            .replace('{movie_id}', video.id.toString())
        });
        setIsFavorite(true);
